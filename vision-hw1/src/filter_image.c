...
{
    float curr; float square = im.w * im.h;
...
image convolve_image(image im, image filter, int preserve)
{
    ...
    if (preserve == 1) {
        ...
        colorize = make_image(im.w, im.h, im.c);
        ...
        for (int ch = 0; ch < im.c; ++ch) {
            for (int i = 0; i < im.h; ++i) {
                for (int j = 0; j < im.w; ++j) {
                    ...
                    set_pixel(colorize, j, i, ch, new_val);
        ...
        colorize = make_image(im.w, im.h, 1);
...
image add_image(image a, image b)
{
    ...
    image colorize = make_image(a.w, a.h, a.c);
    float curr;
    ...
    for (int ch = 0; ch < a.c; ++ch) {
        for (int i = 0; i < a.h; ++i) {
            for (int j = 0; j < a.w; ++j) {
                ...
                set_pixel(colorize, j, i, ch, curr);
            }
        }
    }
...
min_max find_min_max(image im) {
    ...
    for (int ch = 0; ch < im.c; ++ch) {
        for (int i = 0; i < im.h; ++i) {
            for (int j = 0; j < im.w; ++j) {
                curr = get_pixel(im, j, i, ch);
                if (curr < colorize.min)
                    colorize.min = curr;
                if (curr > colorize.max)
                    colorize.max = curr;
            }
        }
    }
...
image *sobel_image(image im)
{
    ...
    image* colorize = calloc(2, sizeof(image));
    colorize[0] = magnitude;
    colorize[1] = direction;
...
image colorize_sobel(image im)
{
    ...
    image colorize = make_image(im.w, im.h, im.c);
    ...
    for (int i = 0; i < im.h; ++i) {
        for (int j = 0; j < im.w; ++j) {
            ...
            set_pixel(colorize, j, i, 0, angle);
            set_pixel(colorize, j, i, 1, mag);
            set_pixel(colorize, j, i, 2, mag);
        }
    }
    ...
    image res = convolve_image(colorize, gauss_filter, 1);
