...
int my_match_compare(const void *a, const void *b)
{
    match *ra = (match *)a;
    match *rb = (match *)b;
...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        for (i = 1; i < bn; ++i) {
            curr_dist = l1_distance(a[j].data, b[i].data, a[j].n);
    ...
    qsort(m, an, sizeof(match), my_match_compare);

    int prev_i = 0;
    int prev_bi = m[prev_i].bi;
...
{  // check later, if errors
    ...
    float new_x, new_y, w;
    new_x = x_prime.data[0][0];
    new_y = x_prime.data[1][0];
    w     = x_prime.data[2][0];

    free_matrix(c);
    free_matrix(x_prime);

    point q = make_point(new_x/w, new_y/w);
...
float point_distance(point p, point q)
{
    ...
    float dist = sqrtf(powf(p.x - q.x, 2) + powf(p.y - q.y, 2));

    return dist;
}


void swap_matches(match* lhs, match* rhs) {
...
// Generates random number in range [a, b)
// int a: left border
// int b: right border
...
{
    printf("In randomize matches\n");

    // TODO: implement Fisher-Yates to shuffle the array.
    int j;
    for (int i = n-1; i >= 0; --i) {
        j = rand_in_range(0, i);
        swap_matches(&m[j], &m[i]);
    }

    printf("After ranfomize matches\n");
    // test this function, because swap or rand may work incorrect
}
...
matrix compute_homography(match *matches, int n)
{
    ...
    printf("After initializing M and b matrices\n");
    ...
    printf("After compute homography\n");
...
matrix RANSAC(match *m, int n, float thresh, int k, int cutoff)
{
    ...
    for (int i = 0; i < k; ++i) {
        ...
        matrix new_H = compute_homography(m, 8);  

        int new_model_inliers = model_inliers(new_H, m, n, thresh);

        if (new_model_inliers > best) {

            best = new_model_inliers;
            Hb = compute_homography(m, new_model_inliers);
...
image combine_images(image a, image b, matrix H)
{
    ...
    for(k = 0; k < a.c; ++k) {
        for(j = 0; j < a.h; ++j) {
            for(i = 0; i < a.w; ++i) {
                // TODO: fill in.
                float a_val = get_pixel(a, i, j, k);
                set_pixel(c, i - dx, j - dy, k, a_val);
            }
        }
    }
